 ...
 * the License.
 */
package com.lc.springboot.mybatisplus.generatecode.config.builder;
...
import com.lc.springboot.mybatisplus.generatecode.config.po.TableField;
import com.lc.springboot.mybatisplus.generatecode.config.po.TableFill;
import com.lc.springboot.mybatisplus.generatecode.config.po.TableInfo;
import com.lc.springboot.mybatisplus.generatecode.config.querys.H2Query;
...
public class ConfigBuilder {
  ...
  private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
    ...
    packageInfo.put(ConstVal.DTO_DETAIL, joinPackage(config.getParent(), config.getDtoResponse()));
    packageInfo.put(
        ConstVal.FEIGN_CLIENT, joinPackage(config.getParent(), config.getFeignClient()));
    packageInfo.put(
        ConstVal.FEIGN_CLIENT_CONST, joinPackage(config.getParent(), config.getFeignClient()));

    // self add
    packageInfo.put(
        ConstVal.PARENT, config.getParent().replace(StringPool.DOT + config.getModuleName(), ""));
    ...
    if (null != configPathInfo) {
      ...
      // self add
      // add update query 同一个包
      setPathInfo(
          pathInfo, template.getDtoAdd(), outputDir, ConstVal.DTO_REQUEST_PATH, ConstVal.DTO_ADD);
      // dto 返回详情
      setPathInfo(
          pathInfo,
          template.getDtoDetail(),
          outputDir,
          ConstVal.DTO_RESPONSE_PATH,
          ConstVal.DTO_DETAIL);
      // feign client
      setPathInfo(
          pathInfo,
  ...
  private List<TableInfo> processTable(List<TableInfo> tableList, StrategyConfig config) {
    ...
    for (TableInfo tableInfo : tableList) {
      ...
      // detail
      if (StringUtils.isNotBlank(globalConfig.getDtoDetailName())) {
        tableInfo.setDtoDetailName(String.format(globalConfig.getDtoDetailName(), entityName));
      } else {
        tableInfo.setDtoDetailName(entityName + ConstVal.DTO_DETAIL);
      }
