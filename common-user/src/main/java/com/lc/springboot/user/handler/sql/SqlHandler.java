...
package com.lc.springboot.user.handler.sql;

import cn.hutool.core.util.ObjectUtil;
import com.baomidou.mybatisplus.annotation.DbType;
import com.baomidou.mybatisplus.core.toolkit.ObjectUtils;
import com.lc.springboot.common.utils.ContextHelper;
import com.lc.springboot.common.utils.S;
import com.lc.springboot.common.utils.SqlExecutor;
...
public class SqlHandler {
    ...
    // 数据字典SQL
    private static final String DICTIONARY_SQL = "SELECT id FROM ${SCHEMA}.sys_dict WHERE id=0";
    ...
    public static void initBootstrapSql(Class inst, Environment environment, String module) {
        ...
        String sqlPath = "sql/init-" + module + "-" + dbType + ".sql";
    ...
    public static boolean extractAndExecuteSqls(Class inst, String sqlPath) {
        ...
        if (ObjectUtil.isEmpty(sqlFileReadLines)) {
            return false;
        }
        // 解析SQL
        ...
        for (String line : sqlFileReadLines) {
            ...
            if (line.contains(";")) {
                ...
                if (line.indexOf(";") < line.length() - 1) {
                    ...
                    if (ObjectUtils.isNotEmpty(leftSql)) {
                        sb.append(leftSql);
                    }
                }
            } else if (ObjectUtils.isNotEmpty(line)) {
                sb.append(line);
            }
        }
        if (sb.length() > 0) {
    ...
    public static boolean executeMultipleUpdateSqls(List<String> sqlStatementList) {
        if (ObjectUtil.isEmpty(sqlStatementList)) {
            return false;
        }
    ...
    protected static List<String> readLinesFromResource(Class inst, String sqlPath) {
        ...
        //去掉注释的语句
        lines = lines.stream().filter(a -> !a.trim().startsWith("#")).collect(Collectors.toList());
        return lines;
    }
    ...
    private static String clearComments(String inputSql) {
        ...
        for (String row : sqlRows) {
            ...
            if (ObjectUtils.isNotEmpty(row.trim())) {
                cleanSql.add(row);
            }
        }
    ...
    public static String getOracleCurrentSchema() {
        if (CURRENT_SCHEMA == null) {
            ...
            if (ObjectUtils.isNotEmpty(alterSessionSql) && S.containsIgnoreCase(alterSessionSql, " current_schema=")) {
